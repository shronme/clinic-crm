...
from app.schemas.service import (
    ...
    ServiceCategory, ServiceCategoryCreate, ServiceCategoryUpdate,
    ServiceAddon, ServiceAddonCreate, ServiceAddonUpdate,
    StaffService, StaffServiceCreate, StaffServiceUpdate
)
from app.services.service import (
    ServiceManagementService, ServiceCategoryService, 
...
async def get_service_category(
    ...
    category = await ServiceCategoryService.get_category(db, category_id, business_id)
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
    return category

@router.put("/categories/{category_id}", response_model=ServiceCategory)
async def update_service_category(
    ...
    category = await ServiceCategoryService.update_category(db, category_id, business_id, category_data)
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
    return category
...
async def delete_service_category(
    ...
    if not await ServiceCategoryService.delete_category(db, category_id, business_id):
        raise HTTPException(status_code=404, detail="Category not found")
...
@router.get("/addons/{addon_id}", response_model=ServiceAddon)
async def get_service_addon(
    ...
    addon = await ServiceAddonService.get_addon(db, addon_id, business_id)
    if not addon:
        raise HTTPException(status_code=404, detail="Add-on not found")
    return addon

@router.put("/addons/{addon_id}", response_model=ServiceAddon)
async def update_service_addon(
    ...
    addon = await ServiceAddonService.update_addon(db, addon_id, business_id, addon_data)
    if not addon:
        raise HTTPException(status_code=404, detail="Add-on not found")
    return addon
...
async def delete_service_addon(
    ...
    if not await ServiceAddonService.delete_addon(db, addon_id, business_id):
        raise HTTPException(status_code=404, detail="Add-on not found")
...
@router.get("/staff-services/{staff_service_id}", response_model=StaffService)
async def get_staff_service(
    staff_service_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Get a single staff-service mapping."""
    mapping = await StaffServiceMappingService.get_staff_service(db, staff_service_id)
    if not mapping:
        raise HTTPException(status_code=404, detail="Staff-service mapping not found")
    return mapping

@router.put("/staff-services/{staff_service_id}", response_model=StaffService)
async def update_staff_service(
    ...
    mapping = await StaffServiceMappingService.update_staff_service(db, staff_service_id, mapping_data)
    if not mapping:
        raise HTTPException(status_code=404, detail="Staff-service mapping not found")
    return mapping

@router.delete("/staff-services/{staff_service_id}")
async def delete_staff_service(
    staff_service_id: int,
    db: AsyncSession = Depends(get_db)
):
    """Delete staff-service mapping."""
    if not await StaffServiceMappingService.delete_staff_service(db, staff_service_id):
        raise HTTPException(status_code=404, detail="Staff-service mapping not found")
...
async def get_services(
    ...
    """Get all services."""
    return await ServiceManagementService.get_services(db, business_id, category_id, is_active)
...
async def get_service(
    ...
    service = await ServiceManagementService.get_service(db, service_id, business_id)
    if not service:
...
async def update_service(
    ...
    """Update service."""
    service = await ServiceManagementService.update_service(db, service_id, business_id, service_data)
    if not service:
        raise HTTPException(status_code=404, detail="Service not found")
    return service
...
async def delete_service(
    ...
    """Delete service."""
    if not await ServiceManagementService.delete_service(db, service_id, business_id):
        raise HTTPException(status_code=404, detail="Service not found")
