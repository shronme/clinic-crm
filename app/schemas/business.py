...
class BusinessBranding(BaseModel):
    ...
    logo_position: Optional[str] = Field(
        "center", description="Logo position on booking portal"
    )
...
class BusinessPolicy(BaseModel):
    ...
    min_lead_time_hours: int = Field(
        1, ge=0, description="Minimum hours before appointment can be booked"
    )
    max_lead_time_days: int = Field(
        90, ge=1, description="Maximum days in advance appointments can be booked"
    )
    cancellation_window_hours: int = Field(
        6, ge=0, description="Hours before appointment when free cancellation ends"
    )
    deposit_required: bool = Field(False, description="Whether deposits are required")
    no_show_fee: Optional[float] = Field(
        None, ge=0, description="Fee charged for no-shows"
    )
    late_arrival_grace_minutes: int = Field(
        15, ge=0, description="Grace period for late arrivals"
    )
...
class BusinessBase(BaseModel):
    ...
    logo_url: Optional[str] = Field(
        None, max_length=500, description="URL to business logo"
    )
    description: Optional[str] = Field(None, description="Business description")
    phone: Optional[str] = Field(
        None, max_length=50, description="Business phone number"
    )
    email: Optional[str] = Field(
        None, max_length=255, description="Business email address"
    )
    ...
    branding: Optional[BusinessBranding] = Field(
        None, description="Branding configuration"
    )
