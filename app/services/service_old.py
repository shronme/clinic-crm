...
class ServiceCategoryService:
    ...
    async def get_categories(db: AsyncSession, business_id: int, parent_id: Optional[int] = None) -> List[ServiceCategory]:
        ...
        query = db.query(ServiceCategory).filter(ServiceCategory.business_id == business_id)
    ...
    @staticmethod
    def get_category(db: AsyncSession, category_id: int, business_id: int) -> Optional[ServiceCategory]:
    ...
    @staticmethod
    def create_category(db: AsyncSession, category_data: ServiceCategoryCreate) -> ServiceCategory:
        """Create a new service category."""
        # Validate parent category exists and belongs to same business
        if category_data.parent_id:
            parent = ServiceCategoryService.get_category(db, category_data.parent_id, category_data.business_id)
            if not parent:
    ...
    @staticmethod
    def update_category(db: AsyncSession, category_id: int, business_id: int, category_data: ServiceCategoryUpdate) -> Optional[ServiceCategory]:
        ...
        if category_data.parent_id:
            ...
            parent = ServiceCategoryService.get_category(db, category_data.parent_id, business_id)
            if not parent:
                raise HTTPException(status_code=400, detail="Parent category not found")
    ...
    def delete_category(db: AsyncSession, category_id: int, business_id: int) -> bool:
        ...
        children = db.query(ServiceCategory).filter(ServiceCategory.parent_id == category_id).first()
        if children:
...
class ServiceManagementService:
    ...
    @staticmethod
    def get_services(db: AsyncSession, business_id: int, category_id: Optional[int] = None, is_active: Optional[bool] = None) -> List[Service]:
    ...
    @staticmethod
    def get_service(db: AsyncSession, service_id: int, business_id: int) -> Optional[Service]:
    ...
    def create_service(db: AsyncSession, service_data: ServiceCreate) -> Service:
        ...
        if service_data.category_id:
            category = ServiceCategoryService.get_category(db, service_data.category_id, service_data.business_id)
            if not category:
    ...
    @staticmethod
    def update_service(db: AsyncSession, service_id: int, business_id: int, service_data: ServiceUpdate) -> Optional[Service]:
        """Update a service."""
        ...
        if service_data.category_id:
            category = ServiceCategoryService.get_category(db, service_data.category_id, business_id)
            if not category:
                raise HTTPException(status_code=400, detail="Category not found")
    ...
    def delete_service(db: AsyncSession, service_id: int, business_id: int) -> bool:
        ...
        staff_services = db.query(StaffService).filter(StaffService.service_id == service_id).first()
        if staff_services:
...
class ServiceAddonService:
    ...
    @staticmethod
    def get_addons(db: AsyncSession, business_id: int, service_id: Optional[int] = None) -> List[ServiceAddon]:
    ...
    @staticmethod
    def get_addon(db: AsyncSession, addon_id: int, business_id: int) -> Optional[ServiceAddon]:
        """Get a single service add-on."""
        return db.query(ServiceAddon).filter(
    ...
    def create_addon(db: AsyncSession, addon_data: ServiceAddonCreate) -> ServiceAddon:
        ...
        service = ServiceManagementService.get_service(db, addon_data.service_id, addon_data.business_id)
        if not service:
    ...
    @staticmethod
    def update_addon(db: AsyncSession, addon_id: int, business_id: int, addon_data: ServiceAddonUpdate) -> Optional[ServiceAddon]:
...
class StaffServiceMappingService:
    ...
    @staticmethod
    def get_staff_services(db: AsyncSession, staff_id: Optional[int] = None, service_id: Optional[int] = None) -> List[StaffService]:
    ...
    @staticmethod
    def get_staff_service(db: AsyncSession, staff_service_id: int) -> Optional[StaffService]:
        """Get a single staff-service mapping."""
        return db.query(StaffService).filter(StaffService.id == staff_service_id).first()

    @staticmethod
    def create_staff_service(db: AsyncSession, mapping_data: StaffServiceCreate) -> StaffService:
    ...
    @staticmethod
    def update_staff_service(db: AsyncSession, staff_service_id: int, mapping_data: StaffServiceUpdate) -> Optional[StaffService]:
