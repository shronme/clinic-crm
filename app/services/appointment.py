...
class AppointmentService:
    ...
    async def create_appointment(
        ...
        validation_request = AppointmentValidationRequest(
            staff_uuid=str((await self._get_staff_by_id(appointment_data.staff_id)).uuid),
            service_uuid=str(service.uuid),
            requested_datetime=appointment_data.scheduled_datetime,
            customer_uuid=str((await self._get_customer_by_id(appointment_data.customer_id)).uuid),
            addon_uuids=[]  # TODO: Handle addons when implemented
        )
        
        validation_response = await self.scheduling_engine.validate_appointment(validation_request)
        
        if not validation_response.is_valid:
            raise ValueError(f"Appointment validation failed: {validation_response.conflicts}")
    ...
    async def get_appointment_by_uuid(self, appointment_uuid: str) -> Optional[Appointment]:
    ...
    async def update_appointment(
        ...
        if (update_data.scheduled_datetime and 
            update_data.scheduled_datetime != appointment.scheduled_datetime):
            ...
            if not validation_response.is_valid:
                raise ValueError(f"Rescheduling validation failed: {validation_response.conflicts}")
            
            # Update estimated end time
            duration = update_data.duration_minutes or appointment.duration_minutes
            appointment.estimated_end_datetime = update_data.scheduled_datetime + timedelta(minutes=duration)
    ...
    async def transition_appointment_status(
        ...
        if transition.new_status == AppointmentStatus.CANCELLED:
            ...
            appointment.cancellation_reason = transition.cancellation_reason.value if transition.cancellation_reason else None
    ...
    async def lock_appointment_slot(
        ...
        success = appointment.lock_slot(
            lock_data.session_id,
            lock_data.lock_duration_minutes
        )
    ...
    async def check_cancellation_policy(
        ...
        if appointment.service:
            ...
            cancellation_window_ends_at = (
                appointment.scheduled_datetime - timedelta(hours=cancellation_window_hours)
            )
    ...
    async def check_appointment_conflicts(
        ...
        query = select(Appointment).where(
            and_(
                ...
                or_(
                    ...
                    and_(
                        Appointment.scheduled_datetime < check.scheduled_datetime + timedelta(minutes=check.duration_minutes),
                        Appointment.estimated_end_datetime >= check.scheduled_datetime + timedelta(minutes=check.duration_minutes)
                    ),
                    and_(
                        Appointment.scheduled_datetime >= check.scheduled_datetime,
                        Appointment.estimated_end_datetime <= check.scheduled_datetime + timedelta(minutes=check.duration_minutes)
        ...
        if has_conflict:
            ...
            base_time = check.scheduled_datetime.replace(hour=9, minute=0, second=0, microsecond=0)
            for day_offset in range(7):
                for hour_offset in range(9):  # 9 AM to 5 PM
                    alternative_time = base_time + timedelta(days=day_offset, hours=hour_offset)
    ...
    async def get_appointment_stats(
        ...
        confirmed_appointments = sum(1 for apt in appointments if apt.status == AppointmentStatus.CONFIRMED.value)
        completed_appointments = sum(1 for apt in appointments if apt.status == AppointmentStatus.COMPLETED.value)
        cancelled_appointments = sum(1 for apt in appointments if apt.status == AppointmentStatus.CANCELLED.value)
        no_show_appointments = sum(1 for apt in appointments if apt.status == AppointmentStatus.NO_SHOW.value)
        
        total_revenue = sum(apt.total_price for apt in appointments if apt.status == AppointmentStatus.COMPLETED.value)
        ...
        average_appointment_value = total_revenue / completed_appointments if completed_appointments > 0 else Decimal('0')
    ...
    async def _validate_entities(
        ...
        business = await self.db.execute(select(Business).where(Business.id == business_id))
        if not business.scalar_one_or_none():
            raise ValueError(f"Business {business_id} not found")
        
        # Check customer exists
        customer = await self.db.execute(select(Customer).where(Customer.id == customer_id))
        if not customer.scalar_one_or_none():
            raise ValueError(f"Customer {customer_id} not found")
        
        # Check staff exists and belongs to business
        staff = await self.db.execute(
            select(Staff).where(and_(Staff.id == staff_id, Staff.business_id == business_id))
        )
        if not staff.scalar_one_or_none():
            raise ValueError(f"Staff {staff_id} not found or doesn't belong to business {business_id}")
        ...
        if not service.scalar_one_or_none():
            raise ValueError(f"Service {service_id} not found or doesn't belong to business {business_id}")
    ...
    def _apply_filters(self, query, filters: AppointmentFilters):
        ...
        if filters.booking_source:
            query = query.where(Appointment.booking_source == filters.booking_source.value)
    ...
    def _apply_search(self, query, search_query: str):
        ...
        # Search in customer name, staff name, service name
        query = query.where(
            or_(
