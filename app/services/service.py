...
class ServiceCategoryService:
    ...
    @staticmethod
    async def get_categories(db: AsyncSession, business_id: int, parent_id: Optional[int] = None) -> List[ServiceCategory]:
        """Get service categories, optionally filtered by parent."""
        stmt = select(ServiceCategory).filter(ServiceCategory.business_id == business_id)
    ...
    @staticmethod
    async def get_category(db: AsyncSession, category_id: int, business_id: int) -> Optional[ServiceCategory]:
        """Get a single service category."""
        stmt = select(ServiceCategory).filter(
            and_(ServiceCategory.id == category_id, ServiceCategory.business_id == business_id)
        )
        result = await db.execute(stmt)
        return result.scalar_one_or_none()

    @staticmethod
    async def create_category(db: AsyncSession, category_data: ServiceCategoryCreate) -> ServiceCategory:
        """Create a new service category."""
        # Validate parent category exists and belongs to same business
        if category_data.parent_id:
            parent = await ServiceCategoryService.get_category(db, category_data.parent_id, category_data.business_id)
            if not parent:
    ...
    @staticmethod
    async def update_category(db: AsyncSession, category_id: int, business_id: int, category_data: ServiceCategoryUpdate) -> Optional[ServiceCategory]:
        """Update a service category."""
        db_category = await ServiceCategoryService.get_category(db, category_id, business_id)
        if not db_category:
            return None
        ...
        if category_data.parent_id:
            ...
            parent = await ServiceCategoryService.get_category(db, category_data.parent_id, business_id)
            if not parent:
                raise HTTPException(status_code=400, detail="Parent category not found")
    ...
    async def delete_category(db: AsyncSession, category_id: int, business_id: int) -> bool:
        ...
        children_stmt = select(ServiceCategory).filter(ServiceCategory.parent_id == category_id)
        ...
        if services_result.first():
            raise HTTPException(status_code=400, detail="Cannot delete category with associated services")
...
class ServiceManagementService:
    ...
    @staticmethod
    async def get_services(db: AsyncSession, business_id: int, category_id: Optional[int] = None, is_active: Optional[bool] = None) -> List[Service]:
    ...
    @staticmethod
    async def get_service(db: AsyncSession, service_id: int, business_id: int) -> Optional[Service]:
    ...
    async def create_service(db: AsyncSession, service_data: ServiceCreate) -> Service:
        ...
        if service_data.category_id:
            category = await ServiceCategoryService.get_category(db, service_data.category_id, service_data.business_id)
            if not category:
    ...
    @staticmethod
    async def update_service(db: AsyncSession, service_id: int, business_id: int, service_data: ServiceUpdate) -> Optional[Service]:
        """Update a service."""
        db_service = await ServiceManagementService.get_service(db, service_id, business_id)
        if not db_service:
            return None

        # Validate category if being updated
        if service_data.category_id:
            category = await ServiceCategoryService.get_category(db, service_data.category_id, business_id)
            if not category:
                raise HTTPException(status_code=400, detail="Category not found")
    ...
    @staticmethod
    async def delete_service(db: AsyncSession, service_id: int, business_id: int) -> bool:
        """Delete a service."""
        db_service = await ServiceManagementService.get_service(db, service_id, business_id)
        if not db_service:
            return False
        ...
        staff_services_stmt = select(StaffService).filter(StaffService.service_id == service_id)
        staff_services_result = await db.execute(staff_services_stmt)
        if staff_services_result.first():
            raise HTTPException(status_code=400, detail="Cannot delete service with staff assignments")
...
class ServiceAddonService:
    ...
    @staticmethod
    async def get_addons(db: AsyncSession, business_id: int, service_id: Optional[int] = None) -> List[ServiceAddon]:
    ...
    @staticmethod
    async def get_addon(db: AsyncSession, addon_id: int, business_id: int) -> Optional[ServiceAddon]:
    ...
    @staticmethod
    async def create_addon(db: AsyncSession, addon_data: ServiceAddonCreate) -> ServiceAddon:
        """Create a new service add-on."""
        # Validate service exists
        service = await ServiceManagementService.get_service(db, addon_data.service_id, addon_data.business_id)
        if not service:
    ...
    @staticmethod
    async def update_addon(db: AsyncSession, addon_id: int, business_id: int, addon_data: ServiceAddonUpdate) -> Optional[ServiceAddon]:
...
class StaffServiceMappingService:
    ...
    @staticmethod
    async def get_staff_services(db: AsyncSession, staff_id: Optional[int] = None, service_id: Optional[int] = None) -> List[StaffService]:
    ...
    @staticmethod
    async def get_staff_service(db: AsyncSession, staff_service_id: int) -> Optional[StaffService]:
    ...
    @staticmethod
    async def create_staff_service(db: AsyncSession, mapping_data: StaffServiceCreate) -> StaffService:
        """Create a new staff-service mapping."""
        # Check if mapping already exists
        existing_stmt = select(StaffService).filter(
            and_(StaffService.staff_id == mapping_data.staff_id, StaffService.service_id == mapping_data.service_id)
        )
        existing_result = await db.execute(existing_stmt)
        if existing_result.first():
            raise HTTPException(status_code=400, detail="Staff-service mapping already exists")
    ...
    @staticmethod
    async def update_staff_service(db: AsyncSession, staff_service_id: int, mapping_data: StaffServiceUpdate) -> Optional[StaffService]:
        """Update a staff-service mapping."""
        db_mapping = await StaffServiceMappingService.get_staff_service(db, staff_service_id)
        if not db_mapping:
            return None
