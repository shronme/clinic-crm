...
def validate_business_branding(branding: dict[str, Any]) -> list[str]:
    ...
    # Validate colors
    if "primary_color" in branding and not validate_hex_color(
        branding["primary_color"]
    ):
        errors.append("primary_color must be a valid hex color (#RRGGBB or #RGB)")

    if "secondary_color" in branding and not validate_hex_color(
...
def validate_business_policy(policy: dict[str, Any]) -> list[str]:
    ...
    if "min_lead_time_hours" in policy:
        if (
            not isinstance(policy["min_lead_time_hours"], int)
            or policy["min_lead_time_hours"] < 0
        ):
            errors.append("min_lead_time_hours must be a non-negative integer")

    if "max_lead_time_days" in policy:
        if (
            not isinstance(policy["max_lead_time_days"], int)
            or policy["max_lead_time_days"] < 1
        ):
            errors.append("max_lead_time_days must be a positive integer")

    if "cancellation_window_hours" in policy:
        if (
            not isinstance(policy["cancellation_window_hours"], int)
            or policy["cancellation_window_hours"] < 0
        ):
    ...
    if "no_show_fee" in policy and policy["no_show_fee"] is not None:
        if (
            not isinstance(policy["no_show_fee"], (int, float))
            or policy["no_show_fee"] < 0
        ):
    ...
    if "late_arrival_grace_minutes" in policy:
        if (
            not isinstance(policy["late_arrival_grace_minutes"], int)
            or policy["late_arrival_grace_minutes"] < 0
        ):
...
def validate_business_data(business_data: dict[str, Any]) -> list[str]:
    ...
    if "logo_url" in business_data and not validate_url_format(
        business_data["logo_url"]
    ):
