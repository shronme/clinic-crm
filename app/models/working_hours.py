from sqlalchemy import Column, Integer, String, DateTime, Time, Boolean, Enum, ForeignKey, Index
...
class WorkingHours(Base):
    ...
    def is_time_available(self, check_time):
        ...
        if (self.break_start_time and self.break_end_time and 
            self.break_start_time <= check_time.time() <= self.break_end_time):
            return False
    ...
    def __repr__(self):
        ...
        return (f"<WorkingHours(id={self.id}, {self.owner_type.value}_id={self.owner_id}, "
                f"{self.weekday.name}: {self.start_time}-{self.end_time}{break_info})>")