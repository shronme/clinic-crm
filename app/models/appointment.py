...
class Appointment(Base):
    ...
    status = Column(
        String(20),
        nullable=False,
        default=AppointmentStatus.TENTATIVE.value,
        index=True,
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "estimated_end_datetime > scheduled_datetime", name="check_end_after_start"
        ),
        CheckConstraint("duration_minutes > 0", name="check_positive_duration"),
        CheckConstraint("total_price >= 0", name="check_non_negative_price"),
        CheckConstraint("deposit_amount >= 0", name="check_non_negative_deposit"),
        CheckConstraint(
            "reschedule_count >= 0", name="check_non_negative_reschedule_count"
        ),
    )
    ...
    def transition_to(
        self, new_status: AppointmentStatus, notes: Optional[str] = None
    ) -> bool:
    ...
    def lock_slot(self, session_id: str, lock_duration_minutes: int = 15) -> bool:
        ...
        self.slot_lock_expires_at = current_time + timedelta(
    ...
    def is_slot_locked(self) -> bool:
        ...
        if self.slot_lock_expires_at and self.slot_lock_expires_at <= datetime.now(
            timezone.utc
        ):
    ...
    def calculate_total_price(
        self, service_price: float, addon_prices: list[float] = None
    ) -> float:
