...
from tests.fixtures.service_fixtures import (
    sample_business, sample_service, sample_service_addon
)
...
class TestServiceAddonService:
    ...
    async def test_get_addons_by_service(self, db: AsyncSession, sample_business: Business):
        ...
        addons = await ServiceAddonService.get_addons(db, sample_business.id, service_id=service.id)
        
        assert len(addons) == 2
        assert all(a.service_id == service.id for a in addons)

    async def test_get_addon_success(self, db: AsyncSession, sample_service_addon: ServiceAddon):
    ...
    async def test_get_addon_not_found(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_get_addon_wrong_business(self, db: AsyncSession, sample_service_addon: ServiceAddon):
        """Test getting an add-on from wrong business."""
        addon = await ServiceAddonService.get_addon(
            db, sample_service_addon.id, 99999  # Wrong business ID
        )
        
        assert addon is None

    async def test_create_addon_success(self, db: AsyncSession, sample_business: Business, sample_service: Service):
    ...
    async def test_create_addon_minimal(self, db: AsyncSession, sample_business: Business, sample_service: Service):
    ...
    async def test_create_addon_invalid_service(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_update_addon_success(self, db: AsyncSession, sample_service_addon: ServiceAddon):
    ...
    async def test_update_addon_partial(self, db: AsyncSession, sample_service_addon: ServiceAddon):
    ...
    async def test_update_addon_not_found(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_delete_addon_success(self, db: AsyncSession, sample_service_addon: ServiceAddon):
    ...
    async def test_delete_addon_not_found(self, db: AsyncSession, sample_business: Business):
        """Test deleting a non-existent add-on."""
        result = await ServiceAddonService.delete_addon(db, 99999, sample_business.id)
        
        assert result is False

    async def test_addon_sorting(self, db: AsyncSession, sample_business: Business, sample_service: Service):
        ...
        addons = await ServiceAddonService.get_addons(db, sample_business.id, service_id=sample_service.id)
