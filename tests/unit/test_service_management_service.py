...
from tests.fixtures.service_fixtures import (
    sample_business, sample_service_category, sample_service, 
    sample_premium_service, multiple_services, sample_staff_service
)
...
class TestServiceManagementService:
    ...
    async def test_get_services_all(self, db: AsyncSession, multiple_services: list[Service]):
    ...
    async def test_get_services_by_category(self, db: AsyncSession, multiple_services: list[Service]):
        """Test getting services filtered by category."""
        business_id = multiple_services[0].business_id
        category_id = multiple_services[0].category_id
        
        services = await ServiceManagementService.get_services(db, business_id, category_id=category_id)
        
        assert len(services) == 3
        assert all(s.category_id == category_id for s in services)

    async def test_get_services_active_only(self, db: AsyncSession, multiple_services: list[Service]):
        """Test getting only active services."""
        business_id = multiple_services[0].business_id
        
        services = await ServiceManagementService.get_services(db, business_id, is_active=True)
    ...
    async def test_get_services_inactive_only(self, db: AsyncSession, multiple_services: list[Service]):
        """Test getting only inactive services."""
        business_id = multiple_services[0].business_id
        
        services = await ServiceManagementService.get_services(db, business_id, is_active=False)
    ...
    async def test_get_service_not_found(self, db: AsyncSession, sample_business: Business):
        """Test getting a non-existent service."""
        service = await ServiceManagementService.get_service(db, 99999, sample_business.id)
        
        assert service is None

    async def test_get_service_wrong_business(self, db: AsyncSession, sample_service: Service):
        """Test getting a service from wrong business."""
        service = await ServiceManagementService.get_service(
            db, sample_service.id, 99999  # Wrong business ID
        )
        
        assert service is None

    async def test_create_service_success(self, db: AsyncSession, sample_business: Business, sample_service_category: ServiceCategory):
    ...
    async def test_create_service_without_category(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_create_service_invalid_category(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_update_service_success(self, db: AsyncSession, sample_service: Service):
    ...
    async def test_update_service_not_found(self, db: AsyncSession, sample_business: Business):
    ...
    async def test_update_service_invalid_category(self, db: AsyncSession, sample_service: Service):
    ...
    async def test_delete_service_success(self, db: AsyncSession, sample_service: Service):
    ...
    async def test_delete_service_not_found(self, db: AsyncSession, sample_business: Business):
        """Test deleting a non-existent service."""
        result = await ServiceManagementService.delete_service(db, 99999, sample_business.id)
        
        assert result is False

    async def test_delete_service_with_staff_assignments(self, db: AsyncSession, sample_service: Service, sample_staff_service):
        ...
        assert exc_info.value.status_code == 400
        assert "Cannot delete service with staff assignments" in str(exc_info.value.detail)

    async def test_service_total_duration_calculation(self, db: AsyncSession, sample_business: Business, sample_service_category: ServiceCategory):
