...
class TestServiceCategoryService:
    ...
    async def test_get_categories_root_only(
        self, db: AsyncSession, multiple_service_categories: list[ServiceCategory]
    ):
        """Test getting root categories only."""
        business_id = multiple_service_categories[0].business_id

        # Get root categories (no parent)
        categories = await ServiceCategoryService.get_categories(
            db, business_id, parent_id=None
        )
    ...
    async def test_get_categories_by_parent(
        ...
        business_id = multiple_service_categories[0].business_id
        hair_category = next(
            cat for cat in multiple_service_categories if cat.name == "Hair Services"
        )

        # Get child categories
        child_categories = await ServiceCategoryService.get_categories(
            db, business_id, parent_id=hair_category.id
        )
    ...
    async def test_get_category_success(
        self, db: AsyncSession, sample_service_category: ServiceCategory
    ):
    ...
    async def test_get_category_wrong_business(
        self, db: AsyncSession, sample_service_category: ServiceCategory
    ):
        """Test getting a category from wrong business."""
        category = await ServiceCategoryService.get_category(
            db,
            sample_service_category.id,
    ...
    async def test_update_category_success(
        ...
        update_data = ServiceCategoryUpdate(
            name="Updated Category", description="Updated description", is_active=False
        )

        updated_category = await ServiceCategoryService.update_category(
            db,
            sample_service_category.id,
            sample_service_category.business_id,
            update_data,
        )
    ...
    async def test_update_category_self_parent(
        self, db: AsyncSession, sample_service_category: ServiceCategory
    ):
    ...
    async def test_update_category_invalid_parent(
        self, db: AsyncSession, sample_service_category: ServiceCategory
    ):
    ...
    async def test_delete_category_with_children(
        self, db: AsyncSession, sample_child_category: ServiceCategory
    ):
        ...
        assert exc_info.value.status_code == 400
        assert "Cannot delete category with child categories" in str(
            exc_info.value.detail
        )

    async def test_delete_category_with_services(
        self, db: AsyncSession, sample_service_category: ServiceCategory, sample_service
    ):
        ...
        assert exc_info.value.status_code == 400
        assert "Cannot delete category with associated services" in str(
            exc_info.value.detail
        )
